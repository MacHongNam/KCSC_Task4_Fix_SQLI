...
<center>
    ...
    <body>
        ...
        if (isset($_POST['register']) && $_POST['username'] != '' && $_POST['password'] != '' && $_POST['repassword'] != '' && $_POST['address'] != '' && $_POST['phonenumber'] != '') {
            ...
            $stmt = $conn->prepare("SELECT * FROM task_2 WHERE username = ?");
            ...
            if ($result->num_rows != 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO task_2 (username, password, address, phonenumber) 
                                        VALUES (?,?,?,?) ");
                $stmt->bind_param("ssss", $username, $password, $address, $phonenumber);
                $stmt->execute();
